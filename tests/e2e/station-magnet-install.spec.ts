...
import { MAGNET_INSTALL_STATION } from './fixtures/station-definitions';
import { runStationHappyPath } from './utils/station-runner';
import { reseedDatabase } from './fixtures/seed';

const STATION_ID = FIXED_IDS.stations.magnet_install;
const WORK_ORDER_ID = FIXED_IDS.workOrders.wo1;

test.describe('Magnet Install Station', () => {
  test.beforeAll(() => {
    reseedDatabase();
  });
  ...
  test('page loads and shows station header', async ({ page }) => {
    ...
    await expect(stationPage.getHeaderName()).toHaveText('Magnet Install');
  });
  ...
  test('happy path — full workflow', async ({ page }) => {
    ...
    const serial = `E2E-MAG-${Date.now()}`;
    await runStationHappyPath(page, MAGNET_INSTALL_STATION, WORK_ORDER_ID, serial);
  });
});
