...
import { ELECTRICAL_TEST_STATION } from './fixtures/station-definitions';
import { runStationHappyPath } from './utils/station-runner';
import { reseedDatabase } from './fixtures/seed';

const STATION_ID = FIXED_IDS.stations.electrical_test;
const WORK_ORDER_ID = FIXED_IDS.workOrders.wo1;

test.describe('Electrical Test Station', () => {
  test.beforeAll(() => {
    reseedDatabase();
  });
  ...
  test('page loads and shows station header', async ({ page }) => {
    ...
    await expect(stationPage.getHeaderName()).toHaveText('Electrical Test');
  });
  ...
  test('happy path — full workflow', async ({ page }) => {
    ...
    const serial = `E2E-ELEC-${Date.now()}`;
    await runStationHappyPath(page, ELECTRICAL_TEST_STATION, WORK_ORDER_ID, serial);
  });
});
